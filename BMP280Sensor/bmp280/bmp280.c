...
uint8_t bmp280_init(void)
{
    ...
    bmp280_setSampling(MODE_NORMAL, SAMPLING_X2, SAMPLING_X16, FILTER_OFF, STANDBY_MS_1);
	return 1;
}
...
void bmp280_setSampling(uint8_t mode, uint8_t tempSampling, uint8_t pressSampling, uint8_t filter, uint8_t duration) {
    bmp280_set_config(duration, filter, 0);
    bmp280_set_ctrl(tempSampling, pressSampling, mode);
}

void bmp280_takeForcedMeasurement(uint8_t mode, uint8_t tempSampling, uint8_t pressSampling) {
    bmp280_set_ctrl(tempSampling, pressSampling, mode);
    while(bmp280_get_status() & 0x08) { // wait until measurement has been completed
        _delay_ms(1);
    }
    bmp280_measure();
}
